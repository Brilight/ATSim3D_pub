...
from Visualize import Temp2d_plot, temp_plot, power_plot, Temp_with_FP, error_plot

home_path = '../2DIC/'
lcfFile = home_path + "Intel_ID1_lcf.csv"
ConfigFile = home_path + "Intel.config"
...
SimParams._sections['Grid']['granularity'] = 1
SimParams._sections['Grid']['rows'] = 64
SimParams._sections['Grid']['cols'] = 64
...
for layer_idx in Local_layer_set:
    if layer_idx in Power_idxs:
        lcf_df.loc[layer_idx, 'Clip_num_x'] = 16
        lcf_df.loc[layer_idx, 'Clip_num_y'] = 16
...
for iter_num in range(iter_leaks):
    ...
    for layer_idx in Local_layer_set:
        ...
        for i in range(grid_rows):
            for j in range(grid_cols):
                result.append(pool.apply_async(FVM_solver, args=(layer.Grid_array[i][j].power,
                   layer.Grid_array[i][j].calc_kappa_init(), layer.Grid_array[i][j].BV_psi, layer.Grid_array[i][j].dxyz)))      
        pool.close()
        pool.join()
        ...
        memory_check()
print("Total takes: ", time.time()-start)
memory_check()