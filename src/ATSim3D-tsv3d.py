...
SimParams._sections['Grid']['rows'] = 40
SimParams._sections['Grid']['cols'] = 40
        ...
        for i in range(grid_rows):
            for j in range(grid_cols):
                result.append(pool.apply_async(FVM_solver, args=(layer.Grid_array[i][j].power,
                                                                 layer.Grid_array[i][j].calc_kappa_init(),
                                                                 layer.Grid_array[i][j].BV_psi, 
                                                                 layer.Grid_array[i][j].dxyz))) 
        memory_check()
        pool.close()
        pool.join()
